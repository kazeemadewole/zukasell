...
$session->start_login();
	...
	$_SESSION['lgarea'] = $lgarea;
	 $url =  $Database->address."Fashion and Beauty/?cat=".$cat."&lgarea=".$_SESSION['lgarea'];
	 $Database->redirect_to($url);
	}
	if(($cat != "") && ($lgarea == "")){
	$_SESSION['lgarea'] = "whole country";
	 $url =  $Database->address."Fashion and Beauty/?cat=".$cat;
	 $Database->redirect_to($url);
	}
	if(($cat == "") && ($lgarea == "")){
	$_SESSION['lgarea'] = "whole country";
	$url =  $Database->address."Fashion and Beauty/?lgarea=".$_SESSION['lgarea'];
	 $Database->redirect_to($url);
	}
	if(($cat == "") && ($lgarea != "")){
	$_SESSION['lgarea'] = $lgarea;
...
<script>
  (adsbygoogle = window.adsbygoogle || []).push({
	...
	<meta charset="utf-8" />
     <title>Zukasell- Fashion and Beauty  </title> 
        <meta name="description" content="Latest Ad in Fashion and Beauty: Watches,Jewelry and Accessories, Clothing and Shoes, Babies and Kids wears, toys ...." /> 
<meta name="keywords" content="Zukasell, fashion, beauty,fashion and beauty,clothing,cloth,watches,watch,shoes,jewelry,babies and kids, health">
...
function validate(e) {
  ...
  if((r != "") && (l != "")){
   window.location = "Fashion and Beauty/?cat="+r+"&lgarea="+l;
  }
  if((r != "") && (l == "")){
	
	 window.location =  "Fashion and Beauty/?cat="+r;

	}
 if((r == "") && (l != "")){
	
	window.location = "Fashion and Beauty/?lgarea="+l;
	}
  }
...
<div class="mainContent">
	...
	padding-bottom:10px">Latest Ads on Fashion & Beauty</h2></header>
	<div class="inside_vehicle" style="background-color: white">
		<div class="list">
<a href="javascript://" onclick="makerequest('<?php echo $Database->address ?>check_watch_brand.php', 'show_hidden')"><li  class="items">Watches</li></a>
<a href="javascript://" onclick="makerequest('<?php echo $Database->address ?>check_cloth_brand.php', 'show_hidden')"><li class="items">Clothing&Shoes</li></a>
<a href="javascript://" onclick="makerequest('<?php echo $Database->address ?>dynamic.php?id=Health%26Beauty &cati=Fashion and Beauty &maker=subcategory&loc=<?php echo $_SERVER['REQUEST_URI'] ?>&cat=Health and Beauty', 'display_box')">
		<li  class="items">Health&Beauty</li>
</a>
<a href="javascript://" onclick="makerequest('<?php echo $Database->address ?>dynamic.php?id=Babies%26kid &cati=Fashion and Beauty &maker=subcategory&loc=<?php echo $_SERVER['REQUEST_URI'] ?>&cat=Babies and Kid', 'display_box')">
		<li class="items">Babies&kid</li>
</a>
<a href="javascript://" onclick="makerequest('<?php echo $Database->address ?>dynamic.php?id=Others &cati=Fashion and Beauty &maker=subcategory&loc=<?php echo $_SERVER['REQUEST_URI'] ?>&cat=Others', 'display_box')" >
...
<?php
$buy = new buyandsell();
$table = "Fashion and Beauty";
$tab = "Fashion-and-Beauty";
...
if ($pag->is_previous_page()){
 echo '<a href="'.$Database->address.$tab.'/?cat='.$cat.'&lg_area='.$_SESSION["lgarea"].'&page='. $pag->previous_page().'">  &laquo Previous </a>';
}

if ($pag->is_next_page()){
 echo '<a style="margin-left:10%" href="'.$Database->address.$tab.'/?cat='.$cat.'&lg_area='.$_SESSION["lgarea"].'&page='. $pag->next_page().'"> Next &raquo </a>';
}
}
echo '</div>';
	   }else{
  ...
  $pag->paging($page,$per_page,$total);
	...
	echo '<div style="clear:both; margin-left:10%; padding-top:10px; ">';
	   if($pag->total_page()  > 1 ){
if ($pag->is_previous_page()){
 echo '<a href="'.$Database->address.$tab.'/?lg_area='.$_SESSION["lgarea"].'&page='. $pag->previous_page().'">  &laquo Previous </a>';
}

if ($pag->is_next_page()){
 echo '<a style="margin-left:10%" href="'.$Database->address.$tab.'/?lg_area='.$_SESSION["lgarea"].'&page='. $pag->next_page().'"> Next &raquo </a>';
}
}
echo '</div>';
	        }
...
echo '</div>';
  ...
  $pag->paging($page,$per_page,$total);
    $offset = $pag->offset(); 
	   ...
	   if($pag->total_page()  > 1 ){
if ($pag->is_previous_page()){
 echo '<a href="'.$Database->address.$tab.'/?page='. $pag->previous_page().'">  &laquo Previous </a>';
}

if ($pag->is_next_page()){
 echo '<a style="margin-left:10%" href="'.$Database->address.$tab.'/?page='. $pag->next_page().'"> Next &raquo </a>';
}
}
echo '</div>';
	      }
