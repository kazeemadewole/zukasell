...
$session->start_login();
	...
	$_SESSION['lgarea'] = $lgarea;
	 $url =  $Database->address."Vehicle/?cat=".$cat."&lgarea=".$_SESSION['lgarea'];
	 $Database->redirect_to($url);
	}
	if(($cat != "") && ($lgarea == "")){
	$_SESSION['lgarea'] = "whole country";
	 $url =  $Database->address."Vehicle/?cat=".$cat;
	 $Database->redirect_to($url);
	}
	if(($cat == "") && ($lgarea == "")){
	$_SESSION['lgarea'] = "whole country";
	$url =  $Database->address."Vehicle/?lgarea=".$_SESSION['lgarea'];
	 $Database->redirect_to($url);
	}
	if(($cat == "") && ($lgarea != "")){
	$_SESSION['lgarea'] = $lgarea;
...
<script>
  (adsbygoogle = window.adsbygoogle || []).push({
	...
	<meta charset="utf-8" />
     <title>Zukasell- Vehicle  </title> 
        <meta name="description" content="Latest Ad in Vehicle: Cars, Motorcycles,Trucks and Commercials, Accessories...." />
<meta name="keywords" content="Zukasell, vehicles, car,truck,bike,motorcycle,commercial bus,price, cost,toyota camry ">
...
function validate(e) {
  ...
  if((r != "") && (l != "")){
   window.location = "Vehicle/?cat="+r+"&lgarea="+l;
  }
  if((r != "") && (l == "")){
	
	 window.location =  "Vehicle/?cat="+r;

	}
 if((r == "") && (l != "")){
	
	window.location = "Vehicle/?lgarea="+l;
	}
  }
...
(adsbygoogle = window.adsbygoogle || []).push({});
   ...
   <div class="mainContent">
	...
	padding-bottom:10px">Latest Ads on Vehicle</h2></header>
	<div class="inside_vehicle" style="background-color: white">
		<div class="list">
		<a href="javascript://" onclick="makerequest('<?php echo $Database->address ?>check_car_brand.php', 'show_hidden')"><li  class="items">Cars</li></a>
		<a href="javascript://" onclick="makerequest('<?php echo $Database->address ?>check_truck_brand.php', 'show_hidden')"><li class="items">Trucks</li></a>
		<a href="javascript://" onclick="makerequest('<?php echo $Database->address ?>motorcycle_brand.php', 'show_hidden')"><li class="items">Motorcycles</li></a>
<a href="javascript://" onclick="makerequest('<?php echo $Database->address ?>dynamic.php?id=Car Accessories &cati=Vehicle &maker=subcategory&loc=<?php echo $_SERVER['REQUEST_URI'] ?>&cat=Car Accessories', 'display_box')">
		<li class="items">Car Accessories</li>
</a>
<a href="javascript://" onclick="makerequest('<?php echo $Database->address ?>dynamic.php?id=Others &cati=Vehicle &maker=subcategory&loc=<?php echo $_SERVER['REQUEST_URI'] ?>&cat=Others', 'display_box')">
