...
$session->start_login();
	...
	$_SESSION['lgarea'] = $lgarea;
	 $url =  $Database->address."Real-Estate/?cat=".$cat."&lgarea=".$_SESSION['lgarea'];
	 $Database->redirect_to($url);
	}
	if(($cat != "") && ($lgarea == "")){
	$_SESSION['lgarea'] = "whole country";
	 $url =  $Database->address."Real-Estate/?cat=".$cat;
	 $Database->redirect_to($url);
	}
	if(($cat == "") && ($lgarea == "")){
	$_SESSION['lgarea'] = "whole country";
	$url =  $Database->address."Real-Estate/?lgarea=".$_SESSION['lgarea'];
	 $Database->redirect_to($url);
	}
	if(($cat == "") && ($lgarea != "")){
	$_SESSION['lgarea'] = $lgarea;
...
<head>
	<meta charset="utf-8" />
     <title>Zukasell- Real Estate  </title> 
        <meta name="description" content="Latest Ad in Real Estate: Land for Rent and Sale, House for Rent and sale, Office Shop and Commercial...." />
<meta name="keywords" content="Zukasell, real estate, rent land, house, house for sale, rent office">
...
function validate(e) {
  ...
  if((r != "") && (l != "")){
   window.location = "Real-Estate/?cat="+r+"&lgarea="+l;
  }
  if((r != "") && (l == "")){
	
	 window.location =  "Real-Estate/?cat="+r;

	}
 if((r == "") && (l != "")){
	
	window.location = "Real-Estate/?lgarea="+l;
	}
  }
...
(adsbygoogle = window.adsbygoogle || []).push({});
   ...
   <div class="mainContent">
	...
	padding-bottom:10px">Latest Ads on Real Estate</h2></header>
	<div class="inside_vehicle" style="background-color: white">
		<div class="list">
<a href="javascript://" onclick="makerequest('<?php echo $Database->address ?>dynamic.php?id=Land &cati=Real Estate &maker=subcategory&loc=<?php echo $_SERVER['REQUEST_URI'] ?>&cat=Land', 'display_box')">
		<li class="items">Land</li>
</a>
<a href="javascript://" onclick="makerequest('<?php echo $Database->address ?>dynamic.php?id=House%26Apartments &cati=Real Estate &maker=subcategory&loc=<?php echo $_SERVER['REQUEST_URI'] ?>&cat=House and Apartments', 'display_box')">
		<li class="items">House&Apartments</li>
</a>
<a href="javascript://" onclick="makerequest('<?php echo $Database->address ?>dynamic.php?id=Office%2CShop%26Commercial &cati=Real Estate &maker=subcategory&loc=<?php echo $_SERVER['REQUEST_URI'] ?>&cat=Office%2CShop and commercial', 'display_box')">
		<li class="items">Office,Shop&Comm</li>
</a>
<a href="javascript://" onclick="makerequest('<?php echo $Database->address ?>dynamic.php?id=Others &cati=Real Estate &maker=subcategory&loc=<?php echo $_SERVER['REQUEST_URI'] ?>&cat=Others', 'display_box')">
