    ...
    $this->file_name6 = basename($Database->escape_value($file6['name']));
     ...
     echo   '<a href="'.$Database->address.'buyandsellad.php"><img src="'.$Database->address.'required/zukasell_ad_sell.jpg" class="no_ad_to_display_image" /></a></div>';         
        ...
        if($this->category == "Fashion & Beauty"){
            $this->pageAdrress = $Database->address."Fashion and Beauty/";
        }elseif($this->category == "Mobile Phones and Tablet"){
            $this->pageAdrress = $Database->address."Mobile Phones and Tablets/";
        }
        elseif($this->category == "Vehicle"){
            $this->pageAdrress = $Database->address."Vehicle/";
        }
         elseif($this->category == "Electronics"){
            $this->pageAdrress = $Database->address."Electronics/";
         }
         elseif($this->category == "Home,Furniture and Garden"){
            $this->pageAdrress = $Database->address."Home Furniture/";
         }
         elseif($this->category == "Real Estate"){
            $this->pageAdrress = $Database->address."Real Estate/";
        }else{
    $this->pageAdrress = $Database->address."Jobs and Services/";
      }
$sanitized_adname = str_replace(" ","-",$this->adname);
	...
	$s= array_rand($a,1);
     ...
     echo '<div class="select_from" >';
      echo '<a href="'.$this->pageAdrress.$this->id."/".$sanitized_adname.$a[$s].'" ><div style="width:100%; border-bottom: 1px solid grey"><img src="'. $source .'" style="width:100%;height: 140px " alt="'.$this->adname.'" /></div></a>';
      echo ' <a href="'.$this->pageAdrress.$this->id."/".$sanitized_adname.$a[$s].'" ><p style="background-color:white; max-height:20px"><strong style=" ">'. $adname .'</strong></p></a>';
      ...
      ?>&#8358
        ...
        echo '<p style="background-color:white; max-height:20px"><strong style=" ">'. $location .'</strong></p>';
...
$source3 = $this->address."uploads/".$user->email."/".$this->file_name3;
    ...
    if($this->category == "Fashion & Beauty"){
    ...
    }elseif($this->category == "Home,Furniture & Garden"){
            $this->Extra_info  = "";
    ...
    }elseif($this->category == "Jobs & Services"){
           $this->Extra_info  = "";
    }
     ...
     . $this->adname .'</strong></p>
      ...
      <img src="'.$source1.'" style="width:100%; max-height:100%" alt="'.$this->adname.'1"/>';
  ...
  echo '</div>';
    ...
    echo '</div>';
      ...
      ?>&#8358
        ...
        <div><a href="'.$Database->address.'conversation/'.$this->id.'/'.$this->user.'/'.$_SERVER['REQUEST_URI'].'"><button  class="button_class">Send Message</button></a></div>
...
<?php
    ...
    public function show_familiar($id){
     global $Database;
     $sqli = "select adname from " .$this->adtable." where id =";
   $sqli .= "'".$id ."'";
   $resi = $Database->add_query($sqli);
   $result = $Database->fetch_array($resi);
   $adrname = $result['adname'];
   
     $sql = "select * from " .$this->adtable." where adname like";
   $sql .= "'"."%".$adrname ."%" ."'";
   $sql .= " and id != ";
   $sql .= "'".$id ."'";
  $sql .= " ORDER BY date DESC";
        ...
        if($numrow > 0){
        echo '<p style="background-color:white;">SIMILAR PRODUCTS</p>';
        }
        while($result = $Database->fetch_array($res)){
            $this->user = $result['user_id'];   
            $this->description = ucfirst($result['description']);
          ...
          $this->quantity = $result['quantity'];
            ...
            $user->find_by_mail($this->user);
            if($this->file_name != ""){
           $source = "m/uploads/".$user->email."/".$this->file_name;
            }
            elseif($this->file_name2 != ""){
            $source = "m/uploads/".$user->email."/".$this->file_name2;
            }
            elseif($this->file_name3 != ""){
            $source = "m/uploads/".$user->email."/".$this->file_name3;
            }else{
            $source = "m/uploads/nopic.jpg";
            }
            if($this->adname == ""){
            $adname = "&nbsp";
            }else{
  ...
  $sql .= " ORDER BY date DESC";
    ...
    $this->pageAdrress = $Database->address."Jobs-and-Services/";
     ...
     echo '<table class="myads_tables" ><tr>';
      ...
      echo ' <td style=" "><strong style=" ">'. $adname .'</strong> <br />
      <strong><img src="'.$this->address .'required/naira.jpg" style="height: 15px"/>'.$this->price .'</strong>
      </td>';
      
       echo'<td><a href="'.$this->pageAdrress.'showad.php?ad_id='.$this->id.'" >Preview</a></td></table>';
